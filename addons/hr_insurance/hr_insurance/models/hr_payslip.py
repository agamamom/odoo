    ...
    def get_inputs(self, contract_ids, date_from, date_to):
        ...
        for record in contract_ids:
            if contract_ids[0]:
                ...
                for result in res:
                    # Tính bảo hiểm thông thường
                    if emp_id.deduced_amount_per_month != 0:
                        if result.get('code') == 'INSUR':
                            result['amount'] = emp_id.deduced_amount_per_month
                    
                    # Bổ sung các bảo hiểm xã hội
                    if result.get('code') == 'BHXH':
                        result['amount'] = emp_id.bhxh_amount
                    elif result.get('code') == 'BHYT':
                        result['amount'] = emp_id.bhyt_amount
                    elif result.get('code') == 'BHTN':
                        result['amount'] = emp_id.bhtn_amount
        return res
